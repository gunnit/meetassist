        ...
        while True:
            ...
            try:
                ...
                if message['type'] == 'audio':
                    ...
                    suggestions = ["This is a test suggestion 1", "This is a test suggestion 2"]
                    await websocket.send_text(json.dumps({
                        'type': 'suggestions',
                        'suggestions': suggestions,
                        'timestamp': datetime.now().isoformat()
                    }))
                    logger.info(f"💡 Sent {len(suggestions)} test suggestions")
...
async def root():
    return {
        "message": "AI Sales Assistant MVP Backend", 
        "status": "running",
